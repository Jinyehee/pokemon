...
export function PokemonProvider({ children }) {
  ...
  const [limit, setLimit] = useState(20); // limit을 state로 관리
    ...
    async function loadAll() {
      try {
        ...
        for (let i = 0; i < batches; i++) {
          const batch = data.results.slice(i * batchSize, (i + 1) * batchSize); // (0~499) (500~999) 이런식으로 500개를 나눠서 요청
  ...
  useEffect(() => {
    if (searchInfo) {
      ...
      const filtered = allPokemons.filter((pokemon) => {
        ...
        const korName =
          pokemon.species?.names?.find((n) => n.language.name === "ko")?.name ||
          "";
      ...
      setList(filtered);
      setLimit(0); // 검색 중에는 무한 스크롤 비활성화
    } else {
      setList([]);
      setOffset(0);
      setLimit(20); // 검색이 끝나면 다시 20개씩 로드
    }
  }, [searchInfo, allPokemons]);

  // 무한 스크롤용 포켓몬 가져오기
  async function fetchPokemons(offset) {
    if (isFetching.current || searchInfo) return; // 검색 중이면 무한 스크롤 비활성화
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !searchInfo && limit > 0) {
